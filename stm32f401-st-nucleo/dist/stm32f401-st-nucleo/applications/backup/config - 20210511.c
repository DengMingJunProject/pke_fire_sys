...
//		return;
//	}
	ef_set_env_blob("config", &config, sizeof(config_t));
}
...
#define BUILDING_ID				"123456789012345"
#define UPDATE_URI				"http://8.129.145.22:8080/pke_fire"
#define VERSION					2021041501
...
rt_err_t config_init(void)
{
	...
	ef_get_env_obj("config", &env);
	ef_read_env_value(&env, &config, sizeof(config_t));
	uint8_t crc = crc8_cal(&config,sizeof(config_t)-1);
	if( crc != config.crc ){
		config_default();
		LOG_E("crc checksum error,set to the default config");
	}
