...
//								nb_iot_publish_smoke(&smoke_stus);
                ...
                else if( type->valueint == CMD_DOWN_STOP ){
                    extern uint8_t     run_stop_flag;
                    run_stop_flag = 0;
                    nb_iot_publish_run_stop(0);
                }
                else if( type->valueint == CMD_DOWN_RUN ){
                    extern uint8_t     run_stop_flag;
                    run_stop_flag = 1;
                    nb_iot_publish_run_stop(1);
                }
			}
...
int nb_iot_publish_startup(void)
{
	...
	#ifdef FIRE_MACHINE
	cJSON_AddItemToObject(root, "fire_machine", cJSON_CreateString("need"));
	#else
	cJSON_AddItemToObject(root, "fire_machine", cJSON_CreateString("no need"));
	#endif
...
int nb_iot_publish_run_stop(uint8_t status)
{
	int ret;
	...
	cJSON_AddItemToObject(root, "device_type", cJSON_CreateNumber(DEVICE_TYPE));
	cJSON_AddItemToObject(root, "building_id", cJSON_CreateString(config.building_id));
	cJSON_AddItemToObject(root, "run_stop", cJSON_CreateNumber(status));
...
{	
    ...
    mqtt_subscribe(client, topic_buf, QOS1, sub_topic_handle1);
    ...
    LOG_I( "The board will be reset at %02d:%02d:%02d" ,check_hour,check_minute,check_second);
...
//            extern uint8_t     run_stop_flag;
//            if( run_stop_flag ){
//                rt_kprintf( "Timming reset the board every day\r\n" );
//                hal_app_reset();
//            }
//		}
